...
void MasterCalendarMain() {
  ...
  if (SubCount == 0) {
    ...
    Img000 = ImgBackground.get(0, 0, 480, 176);
  }
  ...
  if (CalendarMainFlg != -1 && mouseKey == 0) {
    int i = SetData;
  ...
  case 3://記録
    ...
    if (dist(190, 120, mouseX, mouseY) < 12) {
    ...
    if (dist(290, 120, mouseX, mouseY) < 12) {
    ...
    if (dist(180, 148, mouseX, mouseY) < 12) {
    ...
    if (dist(300, 148, mouseX, mouseY) < 12) {
  ...
  //新規作成ボタン
  if (CalendarMainNowTab != 3) MakeNewButton(116, 1, 3);
}
