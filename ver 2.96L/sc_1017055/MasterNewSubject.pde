...
void MasterNewSubject() {  
  ...
  else if (ColorX > 400) ColorX = 400;
  if (ColorY < 260) ColorY = 260;
  else if (ColorY > 580) ColorY = 580;
  ...
  case 1://キャンセル
    ...
    break;
  case 2://完了
    AddLine(FileThemeSubject, "FF"+hex(int(red(ColorNow)), 2)+hex(int(green(ColorNow)), 2)+hex(int(blue(ColorNow)), 2)+","+NewSubjectName);
