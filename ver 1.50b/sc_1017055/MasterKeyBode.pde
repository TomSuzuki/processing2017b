...
void MasterKeyBode() {
  ...
  ftextb(KeyBodeDrawF, 20, 82, TextColor[0], TextColor[2]);
  ftextb(KeyBodeDrawF.substring(0, NowLine), 20, 82, TextColor[1], TextColor[0]);
  ...
  if (kyori(mouseX, mouseY, 450, height-260) < 12) {
  ...
  if (kyori(mouseX, mouseY, 30, height-260) < 12) {
...
//ローマ字変換おかしかったらここで修正。
//上手く変換されなかったらここに条件を追加すれば動くはず
//プログラムに直接これを描くのは頭悪い気がするけどファイルから読み込んで～は...できるな（←なぜやらなかった）
//あー描くじゃなくて書くだ（←書き直す気力ない）
//ここはクラス発表時にはバグ残ってるかもなぁ～（'17/07/05）
//そろそろ購買開く時間かな？（同日 08:32）
//バックアップ取ったらビスケット買いに行くかぁ
//おなかすいたなぁ
//徹夜したけど今日バイトじゃね？
//空きコマ多いしそこで寝るかぁ（昼夜逆転★）
//ってことで購買行ってきます。
//追記：ビスケット2袋買った。
//ローマ字に変換する関数（1文字目からの連続のみカウント）
