...
void MasterSettingHomework() {
  ...
  case 1://キャンセル
    ...
    break;
  case 2://完了
    if (SettingHomeworkFlg == 1) DelLine2("data/schedule/data.csv", SettingHomeworkpData);//修正の部分
  ...
  //マウス
  if (mouseKey == 1 && kyori(mouseX, mouseY, BarX, 564-48) < 28) mouseKey = 3;
  if (mouseKey == 3) {
    BarX = mouseX;
    if (BarX < 40) BarX = 40;
  ...
  if (mouseKey == 3 || kyori(mouseX, mouseY, BarX, 564-48) < 28) {    
  ...
  if (SettingHomeworkFlg == 1) {
    if (DeleteButton()) {
      DelLine2("data/schedule/data.csv", SettingHomeworkpData);
      LoadSchedule("data/schedule/data.csv");   //スケジュールのリロード
      MasterFlg = BeforeFlg;
    }
  }
...
void MasterSelectDate() {
  ...
  if (SelectDateDay < 1) SelectDateDay = getDays(SettingHomeworkYear, SelectDateMonth);
  if (SelectDateDay > getDays(SettingHomeworkYear, SelectDateMonth)) SelectDateDay = 1;
  ...
  textSet(Font001, 24, CENTER, CENTER);
  ftextb((SelectDateDay+getDays(SettingHomeworkYear, SelectDateMonth)-2)%getDays(SettingHomeworkYear, SelectDateMonth)+1+"日", 320, 312-64, TextColor[2], TextColor[0]);
  ftextb((SelectDateDay+1)+"日", 320, 312+64, TextColor[2], TextColor[0]);
  ...
  ftextb(SettingHomeworkYear+"年"+SelectDateMonth+"月"+SelectDateDay+"日"+"（"+WeekDay2[dayweek(SettingHomeworkYear, SelectDateMonth, SelectDateDay)]+"）", 240, 480, TextColor[1], TextColor[0]);
}
...
void MasterClock() {
  ...
  if (mouseFlg%10 == 1) ftextb(str(SelectClockMinute), 240+180*sin(radians(90-SelectClockMinute)*360/60), 320+180*cos(radians(90-SelectClockMinute)*360/60), TextColor[0], TextColor[1]);
  else ftextb(str(SelectClockMinute), 240+180*sin(radians(90-SelectClockMinute)*360/60), 320+180*cos(radians(90-SelectClockMinute)*360/60), MainColor, TextColor[0]);
  ...
  //時間
  if (kyori(mouseX, mouseY, 240+180*sin(radians((90-SelectClockMinute)*360/60)), 320+180*cos(radians((90-SelectClockMinute)*360/60))) < 24) {
  ...
  if (mouseFlg == 11) SelectClockMinute = 60-(int(degrees(atan2(mouseX-240, mouseY-360)))*60/360+30)%60;
  SelectClockHour = SelectClockHour % 12;
  //分
  if (kyori(mouseX, mouseY, 240+100*sin(radians((18-SelectClockHour)*360/12)), 320+100*cos(radians((18-SelectClockHour)*360/12))) < 24) {
  ...
  if (mouseFlg == 12) SelectClockHour = 12-(int(degrees(atan2(mouseX-240, mouseY-360)))*12/360+6)%12;
  ...
  textSet(Font001, 36, CENTER, CENTER);
  if (kyori(mouseX, mouseY, 80, 600) < 48) {
    ftextb(SelectAMPM[Select12Flg], 80, 600, MainColor, TextColor[0]);
    if (mouseKey == 1) {
      mouseKey = 2;
